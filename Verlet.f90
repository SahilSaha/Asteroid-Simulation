program trajectory
    ...
    do i = 1,52000
        ...
        x(i+1) = x(i) + u(i)*dt + 0.5*ax(x(i),y(i),u(i),v(i))*(dt**2)
		y(i+1) = y(i) + v(i)*dt + 0.5*ay(x(i),y(i),u(i),v(i))*(dt**2)
		
		u(i+1) = u(i) + 0.5*(ax(x(i),y(i),u(i),v(i))+ax(x(i+1),y(i+1),u(i),v(i)))*dt
		v(i+1) = v(i) + 0.5*(ay(x(i),y(i),u(i),v(i))+ax(x(i+1),y(i+1),u(i),v(i)))*dt
		
		p(i) = -0.5*(w**2)*(x(i)**2+y(i)**2) - (G*ms)/(sqrt((x(i)+rs)**2+y(i)**2)) - (G*mj)/(sqrt((x(i)-rj)**2+y(i)**2))

    end do

    open(unit = 10, file = 'VerletP.txt')
